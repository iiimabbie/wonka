...
func handleAdminAgents(c echo.Context) error {
	...
	for rows.Next() {
		var e entry
		rows.Scan(&e.ID, &e.Name, &e.Enabled, &e.Balance, &e.OwnerID, &e.OwnerEmail)
...
func handleAdminUsers(c echo.Context) error {
	...
	for rows.Next() {
		var e entry
		rows.Scan(&e.ID, &e.Email, &e.Name, &e.Role, &e.AgentCount)
...
func handleAdminDeleteUser(c echo.Context) error {
	...
	// Check user exists
	var exists bool
	pool.QueryRow(context.Background(), `SELECT EXISTS(SELECT 1 FROM users WHERE id = $1)`, userID).Scan(&exists)
	if !exists {

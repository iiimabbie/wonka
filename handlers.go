...
func handleTransfer(c echo.Context) error {
	...
	// Transaction with advisory lock to prevent balance race conditions
	...
	// Lock both agents in consistent order to avoid deadlock
	lockFirst, lockSecond := agent.ID, toID
	if agent.ID > toID {
		lockFirst, lockSecond = toID, agent.ID
	}
	if _, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock($1)`, agentLockID(lockFirst)); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "db error"})
	}
	if _, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock($1)`, agentLockID(lockSecond)); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "db error"})
	}

	// Balance check inside transaction (under lock)
	var balance int
